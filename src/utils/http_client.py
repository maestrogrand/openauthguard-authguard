...
async def fetch_user(username: str, users_service_url: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.debug(f"Received response from Users service: {response.status_code} - {response.text}")
            response.raise_for_status()
            user_data = response.json()
            logger.info(f"Successfully fetched user details for username: {username}")
            return user_data
    except httpx.HTTPStatusError as e:
        logger.error(f"HTTP error while fetching user details: {e.response.status_code} - {e.response.text}")
...
async def fetch_tenant(company_name: str, users_service_url: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(url)
            logger.debug(f"Received response from Tenant service: {response.status_code} - {response.text}")
            response.raise_for_status()
            tenant_data = response.json()
            logger.info(f"Successfully fetched tenant details for company: {company_name}")
            return tenant_data
    except httpx.HTTPStatusError as e:
        logger.error(f"HTTP error while fetching tenant details: {e.response.status_code} - {e.response.text}")
        raise HTTPException(status_code=e.response.status_code, detail=e.response.text)
    except Exception as e:
